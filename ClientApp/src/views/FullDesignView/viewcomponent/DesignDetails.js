...
const DesignDetails = (props) => {
    // ** Props
    const {View3dPlugin, PluginModel, selectedImgDataForDetail, data, deleteWishlistItem, dispatch, addToWishlist, getProduct, productId, addToCart, designMaster } = props
    // console.log(selectedImgDataForDetail, designMaster, 'vaibh2')
    ...
    // const supplierID = () => {
    //     const supplierID = JSON.parse(localStorage.userData).org_type_id
    //     return supplierID
    // }
    // const q3dUrlLink = () => {
    //     const q3dUrlLink = selectedImgDataForDetail && selectedImgDataForDetail.designCode ? selectedImgDataForDetail.colorwayDesigns.length > 0 ? selectedImgDataForDetail.colorwayDesigns[0].q3dUrl : "" : ""
    //     return q3dUrlLink
    // }
    // const selectedDesignName = () => {
    //    const selectedDesignName = selectedImgDataForDetail && selectedImgDataForDetail.designCode ? props.selectedImgDataForDetail.designCode  : 'unknown'
    //    return selectedDesignName
    // }
    ...
    return (
        <div>
           {/* <div className='qrBox  position-absolute' onClick={() => { $(".item-features").toggle(); $(".qrBox").toggleClass("detailOpened") }} >*/}
           ...
           {/*</div>*/}
           <div className='item-features' style = {{display: 'none'}}>
                <Row className='text-left mx-1'>
                    <Col className='mb-4 mb-md-0' xs='12'>               
                        <h4>Design : {selectedImgDataForDetail && selectedImgDataForDetail.designName ? props.selectedImgDataForDetail.designName  : 'unknown'}</h4>
                            {/* <div className='ecommerce-details-price d-flex flex-wrap mt-1'>
                            <h4 className='item-price mr-1'>Rs. {selectedImgDataForDetail && selectedImgDataForDetail.price ? selectedImgDataForDetail.price : selectedImgDataForDetail && selectedImgDataForDetail.features && selectedImgDataForDetail.features['Price'] ? selectedImgDataForDetail.features['Price'] : 0}</h4>                        
                            <div className='pl-1 border-left'><DesignRatings isThumb={true} rating={selectedImgDataForDetail && selectedImgDataForDetail.features["Rating"]}/></div>
                            </div> */}
                           {/*  <CardText className='pt-1'>
                                Stock Availabilty-<span className='text-success ml-25'>: {selectedImgDataForDetail && selectedImgDataForDetail.features["Stock"]} mtrs</span>
                            </CardText>                  
                            <CardText tag='span' className='item-company'>
                               {selectedImgDataForDetail && selectedImgDataForDetail.design_Desc}
                            </CardText> */}
                            ...
                            {access["222229"] && access["222229"]["288889"] && <li onClick={() => {
                                    ...
                                    axios.post('./DesignSearch/DownloadDesignSpecification', obj).then(e => {
                                     ...
                                     <span className='cursor filedownload'><FileText size={19} className ='text-secondary'/></span>
                ...
                selectedImgDataForDetail && selectedImgDataForDetail.designInfo && Object.keys(selectedImgDataForDetail.designInfo).map((e, k) => {
                    return  (<tr key={k}> 
                        ...
                        <td><span className='propcolon'>:</span></td>
            ...
            {access["222229"] && access["222229"]["298889"] && <div className='mb-4 mb-md-0 viewColorways' md='4' xs='12'>
                <div className='w-95' id='colourwaysDiv'>
                   {/* <h4 className='mb-1'>Colorways</h4>*/}
                    <DesignColorways View3dPlugin={View3dPlugin} setActiveImageData={props.setActiveImageData}
                        PluginModel={PluginModel}
                        colorwayDesigns={selectedImgDataForDetail ? selectedImgDataForDetail ? selectedImgDataForDetail : window.colorways : []} />
                </div>
            </div>}
        </div>
    )
