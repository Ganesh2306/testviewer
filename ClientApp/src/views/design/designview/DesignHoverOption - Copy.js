...
import { ShoppingCart, Heart, Book, Check, X, Plus, Trash, Minus, ArrowDownCircle, ArrowDown, ChevronDown, ShoppingBag, CheckCircle } from 'react-feather'
import { Badge, Button, Card, Row, Input, Form, Col, Label } from 'reactstrap'
...
const QuantityOrder = (props) => {
    ...
    const handleSubmitQty = e => {     
        ...
        props.designData.myrequest = requestType || "Reference"
        ...
        props.notifySuccess()
        // e.currentTarget.parentElement.parentElement.parentElement.classList.add('cartSuccess') 
    ...
    return (    
       ...
       <button type="button" class="close" onClick={() => props.setCartbox(false)}><span ><X/></span></button>
            <Form className='p-1'>
            <Row className="m-0">
...
<Col className='mb-1' md='12' sm='12'>
  <Label for='IconsMobile'>Order Quantity</Label>
  <Input 
  type='text' 
  className='rounded-0' 
  name='text' 
...
<Label for='IconsMobile'>Comments</Label>
  ...
  />  
           ...
           <div className='d-flex'> {moveinorder ?  <><button className="btn btn-sm w-100 countaply rounded-0 btn-primary border-none" onClick={handleSubmitQty} ><ShoppingCart size={20} className='mr-1'/>Add to Cart</button></> : <><Button className="btn btn-sm w-50 countaply addedCart rounded-0" onClick={showCart} ><CheckCircle size={20} className='mr-50'/>View in Cart</Button>  <div className="showoverlayQty">{fabqty} mtrs ordered</div></> }    </div>
            </div>   
            {/* {!moveinorder ? <div className="showoverlayQty">{fabqty} mtrs ordered</div> : ''} */}
 </>
    )
}
// const SuccessCartBlink = (props) =>{
    
//     return (
//         <>
//         {props.moveinorder ? <div className="cartSuccess_child"></div> :''}
//         </>
//     )
// }
...
const Favhoverbox = (props) => {
    ...
    return (
        <>
           ...
           onClick={() => props.name ? setis_open(true) : setis_createopen(true)} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}><ChevronDown size={20} />
           <div className="verticalHover">{is_boarduser && !selectedUser ? <span>Collections</span> : <span>Wishlists</span>}</div>
            ...
            {!isHovered ? <Card className="favhover text-left flex-column px-50" onClick={() => props.name ? setis_open(true) : setis_createopen(true)}>
                ...
                {props.bcMenudata ? <div className="d-flex justify-content-between mt-50 boardn">
                            <small className="font-weight-bold coll_wishname" style={{ textTransform: "capitalize" }} >
                                -{props.name} 
                            </small>
                ...
                {/* {props.bcMenudata.length === 0 ? <small className="NewWishlist">New Wishlist</small>:''} */}  
            </Card> : ''}
                    ...
                    {/*    setis_createopen={CreateOpentoggle} setBoardSelected={props.setBoardSelected} />*/}
                    <ModalCreateWishlist seticonActive={props.seticonActive}
                        iconfavstate={props.iconfavstate} is_createopen={is_createopen}
...
const Coloroverlay = ({ colourWay, colourWayClick,
    ...
    const NewcolourWay = (designID, _colourWay, _ptDesign) => {
        ...
        //const pt = ptDesign
        //delete pt.colorwayDesigns
        //arr.push(pt)
        //const new_colour = arr.filter((e) => e.designId !== designID)
        //arr.push(_ptDesign)
        //console.log(arr)
...
const WrapBtn = ({ boardId, iconfavstate, lastoptions, board, len, is_boarduser, BoardSuccess, idtosend, seticonActive, seticonfavActive, ptDesign, access, setBoardSelected, setforloader, CollectionSuccess }) => {
    ...
    const addOrDelete = () => {
        ...
        if (lastoptions) {
            ...
            //designDetailsDto.boardName = lastoptions //`tds` //props.ptDesign.boardName
            if (is_boarduser) {
                //collectionName
                //designDetailsDto.collectionName = lastoptions 
            } else {
                //designDetailsDto.boardName = lastoptions
            }
            //props.ptDesign.boardName
            //designDetailsDto. = props.ptDesign.designCode
            if (is_boarduser) {
                if (selectedUser) {
                    boardDetailDto.Bd_Supplier_Id = JSON.parse(localStorage.selectUserBoard).allFavoriteList.find(p => p.favorite_Id == lastoptions).board_Supplier_Id
                ...
                boardDetailDto.Bd_Supplier_Id = board.find(p => p.favorite_Id == lastoptions).board_Supplier_Id
                boardDetailDto.Board_id = idtosend
                boardDetailDto.bd_Dm_Id = ptDesign.designId
                boardDetailDto.State = iconfavstate ? 3 : 0
            }
    ...
    >
        ...
        {/* {iconfavstate ? <> <Heart  className ='addPlus' size={18} /><Minus className ='removeMinus'size={18} /></>  : <><Plus size={18} /> <span className='align-middle'>Save</span></>} */}
    </Button> : <button
        onClick={addOrDelete}
        //className={iconfavstate ? "iconthumb btn-wishlist iconstate active bg-primary" : "iconthumb btn-wishlist iconstate"}
...
const DesignHoverOption = (props) => {
    ...
    //let idtosend = board && (len === 0 && board ? 0 : board[0].favorite_Id ? board[0].favorite_Id : board[0].collection_Id)
    let idtosend = props.bID
    let favState = undefined
    let lastoptions = idtosend
    let showDesignHover = false
    //sessionStorage.setItem("privselected", lastoptions)
    ...
    if (boardId) {
        ...
        if (BC_Menu.value) {
            ...
            if (history.location.state == "col") {
                ...
                if (is_boarduser) {
                    lastoptionsName = len === 0 && bcMenudata ? "" : bcMenudata[boardId].board_Name ? bcMenudata[boardId].board_Name : bcMenudata[boardId].collection_Name //board && board.allFavoriteList && board.allFavoriteList[len - 1].board_Name
                    idtosend = len === 0 && bcMenudata ? 0 : bcMenudata[boardId].favorite_Id ? bcMenudata[boardId].favorite_Id : bcMenudata[boardId].collection_Id//collection_Id
                    lastoptions = idtosend
        ...
        lastoptionsName = board && (len === 0 && board ? "" : board[0].board_Name ? board[0].board_Name : board[0].collection_Name) //board && board.allFavoriteList && board.allFavoriteList[len - 1].board_Name
        idtosend = board && (len === 0 && board ? 0 : board[0].favorite_Id ? board[0].favorite_Id : board[0].collection_Id)//collection_Id
    ...
    toast.success(<BoardSuccessToast  {...data}/>, { hideProgressBar: true, autoClose: false })
    }
    ...
    return (
        <>  
            ...
            onClick = {(e) => { 
                     togglecartbox()
                     if (setCartbox) {           
                      document.querySelectorAll('#Filter-main-viewPage .ecommerce-card.cartOpened').forEach(e => {
                        e.classList.remove('cartOpened')
                      })
                      e.currentTarget.parentElement.parentElement.classList.add('cartOpened')
        ...
        { !is_boarduser || (is_boarduser && selectedUser) ? ( props.cartbox && <QuantityOrder  notifySuccess={notifySuccess} iconfavstate={iconfavstate} setCartbox = {props.setCartbox} cartbox={props.cartbox} designData={props.ptDesign} setBoardSelected={props.setBoardSelected} /> ) : <></> }
                ...
                {/*    className={iconfavstate ? "iconthumb btn-wishlist iconstate" : "iconthumb btn-wishlist iconstate"} onClick = {() =>{*/}

                {/*        }} >*/}
                {/*        <Trash size={18} className='mr-0' />*/}
                {/*  </button>  : <></> }  /*Added By : Vijay Pansande, Added On : 14-12-2022, Purpose : Delete is not working for supplier user
                 !is_boarduser && boardId */   }
