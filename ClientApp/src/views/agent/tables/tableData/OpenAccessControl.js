...
const AccessControlAgent = (props) => {
            ...
            .then(response => {
                ...
                if (rolesData !== null && rolesData !== undefined) {
                    ...
                    if (rolesData.length > 0) {
                        const userId = document.getElementById('AAC_agtName').getAttribute('uid')
                        axios.get(`./Role/GetRoleAssignmentByUserId?userId=${userId}`)
                            .then(response => {
                                ...
                                if (role_Assignments !== null && role_Assignments !== undefined) {
                                    ...
                                    document.getElementById('AAC_agtName').setAttribute('raid', response.data.role_Assignments.role_Assignment_Id)
                                    if (document.querySelector(`option[roleid="${role_Assignments.role_Id}"]`) !== null) {
                                        ...
                                        document.getElementById('AAC_accessRole').value = selectedValue
                                    }
                                }
                            })
                    }
    ...
    function onSubmit(e) {
        ...
        _RoleAssignment.user_Id = document.getElementById('AAC_agtName').getAttribute('uid')

        _RoleAssignment.is_Role_Admin = true
        _RoleAssignment.role_assignment_id = document.getElementById('AAC_agtName').getAttribute('raid') === null ? 0 : document.getElementById('AAC_agtName').getAttribute('raid')
        const selectedRoleID = document.getElementById('AAC_accessRole').selectedOptions[0].getAttribute('roleid')
        if (Number(selectedRoleID) === 0) {
            _RoleAssignment.state = 3
        } else {
            _RoleAssignment.state = document.getElementById('AAC_agtName').getAttribute('raid') === null ? 0 : 2
        }
    ...
    return (
        <div>
            <div className='d-flex' onClick={() => getRoleByType(3)}>
                <Button.Ripple color='' className='btn-sm'>
                    <Edit size={15} />
                </Button.Ripple>
            </div>
            <Modal backdrop="static" isOpen={is_open} toggle={() => setis_open(false)} className='modal-md modal-dialog-centered'>
                <ModalHeaderUI setis_open={setis_open} headerName=" Access Control" />
                <Form onSubmit={onSubmit}>
                    <ModalBodyUI agentData={props.agentData} setRole={setRole} bodyFor="AccessControlAgent" />
...
export default AccessControlAgent
